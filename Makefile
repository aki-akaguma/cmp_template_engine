...
BENCH_STR_3 = --bench=bench-teams-so
...
bench-gnu: bench.1-gnu bench.2-gnu bench.3-gnu

bench-musl: bench.1-musl bench.2-musl bench.3-musl
...
bench.3-gnu:
	@rm -f z.bench.3.log
	$(ENV) cargo bench --no-run $(TARGET_GNU)
	$(ENV) $(TASKSET) cargo bench $(BENCH_STR_3) $(TARGET_GNU) -- -n | tee -a z.bench.3.log
...
bench.3-musl:
	@rm -f z.musl.bench.3.log
	$(ENV) cargo bench --no-run $(TARGET_MUSL)
	$(ENV) $(TASKSET) cargo bench $(BENCH_STR_3) $(TARGET_MUSL) -- -n | tee -a z.musl.bench.3.log
